...
候補ソースが英語でも、Slack に表示するタイトル、要約は日本語に翻訳・要約して投稿する。
サービス名、会社名、API 名、モデル名などの固有名詞は原語のまま残す。

- タイトル
- 要約（180〜240文字くらい。100文字未満にせず、何が変わったか、誰に影響するか、チームで確認すべき観点を含める）
- 公式リンク
...
Slack ではカテゴリ別に投稿される。
各カテゴリ投稿の先頭には区切り線が入り、カテゴリの切り替わりが分かるようにしている。

```text
━━━━━━━━━━━━━━━━━━━━
フロントエンドニュース - 2026/06/26(金)
```
